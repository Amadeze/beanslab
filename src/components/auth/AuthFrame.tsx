...
export function AuthFrame({
  ...
  return (
    <main className="instrument-grid-dark relative min-h-[100dvh] overflow-hidden bg-[#080B0C] p-3 text-white sm:p-5 lg:p-8">
      {!reduceMotion ? (
        <motion.div
          data-testid="auth-ambient-scan"
          className="pointer-events-none absolute inset-y-0 w-40 bg-gradient-to-r from-transparent via-[#15B8C6]/[0.045] to-transparent blur-xl"
      ...
      <motion.div
        className="relative mx-auto grid min-h-[calc(100dvh-1.5rem)] max-w-[1180px] overflow-hidden rounded-[20px] border border-white/10 bg-[#111617] shadow-[0_32px_120px_rgba(0,0,0,.42)] sm:min-h-[calc(100dvh-2.5rem)] lg:min-h-[calc(100dvh-4rem)] lg:grid-cols-[minmax(0,.9fr)_minmax(440px,.65fr)]"
      ...
      >
        <aside className="relative hidden overflow-hidden border-r border-white/10 p-10 lg:flex lg:flex-col lg:justify-between">
          {!reduceMotion ? (
            <motion.div
              className="pointer-events-none absolute -left-28 top-1/3 size-72 rounded-full bg-[#B65331]/10 blur-[90px]"
          ...
          <div className="relative z-10">
            ...
            >
              <Link href="/" className="inline-flex items-center gap-3">
                <span className="relative flex size-11 items-center justify-center rounded-[11px] bg-[#B65331] text-white">
            ...
            <div className="mt-20 max-w-lg">
              <motion.p
                className="font-mono text-[9px] font-bold uppercase tracking-[0.22em] text-[#15B8C6]"
          ...
          <div className="relative z-10">
            <div className="grid grid-cols-5 gap-1" aria-hidden>
              {["#2B7567", "#B65331", "#A66F12", "#6F4A6A", "#4B6B3C"].map(
                (color, index) => (
                  <motion.span
                    key={color}
        ...
        <motion.section
          className="flex min-w-0 flex-col bg-[#F4F2EA] text-[#141817]"
          initial={reduceMotion ? false : { opacity: 0, x: 24 }}
          animate={reduceMotion ? undefined : { opacity: 1, x: 0 }}
          transition={{ duration: 0.62, delay: 0.16, ease }}
        >
          <div className="flex items-center justify-between border-b border-[#CDC8BC] px-5 py-4 lg:hidden">
            <Link href="/" className="inline-flex items-center gap-2.5">
              <span className="relative flex size-9 items-center justify-center rounded-[9px] bg-[#B65331] text-white">
                <Coffee size={16} />
                <span className="absolute -right-1 -top-1 size-2 rounded-full bg-[#15B8C6]" />
              </span>
            ...
            <motion.span
              ...
              animate={
                reduceMotion
                  ? undefined
                  : { boxShadow: ["0 0 0 #15B8C600", "0 0 16px #15B8C6CC", "0 0 0 #15B8C600"] }
              }
