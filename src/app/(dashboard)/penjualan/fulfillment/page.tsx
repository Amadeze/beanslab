    ...
    <div className="custom-scrollbar flex-1 overflow-auto"><div className="mx-auto max-w-6xl p-4 md:p-6 lg:p-8">
      {invoices.length === 0 ? <div className="rounded-xl border border-dashed border-stone-300 bg-white p-10 text-center text-sm text-stone-500">Tidak ada pesanan storefront yang perlu ditindaklanjuti.</div> : <div className="grid gap-3">
        {invoices.map((invoice) => <article key={invoice.id} className="grid gap-4 rounded-xl border border-stone-200 bg-white p-4 md:grid-cols-[1fr_auto] md:items-center">
          ...
          <div className="flex flex-wrap gap-2">{invoice.fulfillmentTasks.length ? invoice.fulfillmentTasks.map((task) => <Link key={task.id} href={task.product.materialOrigin === "INTERNAL_ROAST" ? `/roasting?productId=${encodeURIComponent(task.product.id)}` : "/inventory"} className="inline-flex h-9 items-center rounded-lg bg-amber-700 px-3 text-xs font-bold text-white">{task.product.materialOrigin === "INTERNAL_ROAST" ? "Buka roasting" : "Terima stok"}</Link>) : null}<Link href="/penjualan" className="inline-flex h-9 items-center rounded-lg border border-stone-300 px-3 text-xs font-bold">Buka pesanan</Link></div>
        </article>)}
      </div>}
    </div></div>
  </div>;
