...
export default async function PaymentReviewPage() {
  ...
  return <div className="flex min-h-0 flex-1 flex-col"><PageHeader title="Bukti Pembayaran" eyebrow="Penjualan" description={`${pending} bukti menunggu keputusan. Hanya bukti yang disetujui mencatat pembayaran dan jurnal kas.`} actions={<Link href="/keuangan?tab=piutang" className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-border bg-card/60 px-3 text-xs font-semibold text-ink transition hover:bg-card">Terima pembayaran piutang →</Link>} /><WorkspaceNav kind="sales" /><div className="custom-scrollbar flex-1 overflow-auto"><div className="mx-auto max-w-6xl p-4 md:p-6 lg:p-8"><PaymentReviewClient rows={serialized} /></div></div></div>;
}
