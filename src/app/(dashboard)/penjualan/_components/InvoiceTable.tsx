...
import { deriveInvoiceStage, deriveInvoiceNextAction } from "@/lib/workflow";
...
export function InvoiceTable({ invoices }: { invoices: InvoiceRow[] }) {
  ...
  const dispatchNext = (inv: InvoiceRow) => {
    const next = deriveInvoiceNextAction(inv);
    switch (next.mode) {
      case "approve":
        handleApprove(inv);
        break;
      ...
      case "fulfill":
        setResiTarget(inv);
        break;
      // prepare ΓåÆ NextAction renders a Link, navigates on its own
    }
  };
            ...
            filteredInvoices.map((inv) => {
              ...
              <TableRow key={inv.id} className="transition-colors hover:bg-surface-sunken">
                ...
                <TableCell className="text-center">
                  <WorkflowState {...deriveInvoiceStage(inv)} />
                </TableCell>
                <TableCell className="text-center">
                  <div className="flex items-center justify-center gap-1.5">
                    {next.mode && (
                      <NextAction
                        ...
                        tone={next.tone}
                        href={next.href}
                        onClick={next.href ? undefined : () => dispatchNext(inv)}
