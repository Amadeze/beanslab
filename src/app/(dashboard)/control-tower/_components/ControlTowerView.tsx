  ...
  return (
    <CardHeader className="flex-row items-start justify-between gap-3">
      <div className="flex min-w-0 gap-3">
        <span className="mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-xl bg-[var(--chrome-panel)] text-copper">
          <Icon size={17} />
        </span>
      ...
      <div className="space-y-5 p-4 md:p-6">
        ...
        <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
          ...
          <Card>
            ...
            <ul className="divide-y divide-border">{data.readiness.map((check) => <li key={check.key} className="flex gap-3 px-5 py-3"><span className={`mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full ${check.ready ? "bg-[color-mix(in srgb, var(--status-success) 12%, white)] text-[var(--status-success)]" : "bg-[color-mix(in srgb, var(--status-warning) 15%, white)] text-[var(--status-warning)]"}`}>{check.ready ? <BadgeCheck size={14} /> : <AlertTriangle size={13} />}</span><div className="min-w-0 flex-1"><Link href={check.href} className="text-xs font-bold text-ink hover:underline">{check.label}</Link><p className="mt-0.5 text-[11px] leading-4 text-ink-secondary">{check.detail}</p></div></li>)}</ul>
          </Card>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          <Card>
            ...
            <div className="grid grid-cols-3 gap-2 p-4 text-center"><div><strong className="text-xl text-ink">{data.warehouse.expiringLots.length}</strong><p className="text-[10px] text-ink-tertiary">lot ≤30 hari</p></div><div><strong className="text-xl text-ink">{data.warehouse.unplacedLotCount}</strong><p className="text-[10px] text-ink-tertiary">belum ditempatkan</p></div><div><strong className="text-xl text-[var(--status-danger)]">{data.warehouse.latePoCount}</strong><p className="text-[10px] text-ink-tertiary">PO terlambat</p></div></div>
