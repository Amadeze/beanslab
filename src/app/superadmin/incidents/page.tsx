  ...
  return (
    <div className="mx-auto flex max-w-[1480px] flex-col gap-6 p-5 md:p-8">
      <header className="flex flex-col justify-between gap-4 border-b border-border pb-6 md:flex-row md:items-end">
        <div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-domain-production">System observability</p><h1 className="mt-3 text-3xl font-black tracking-[-0.05em] md:text-4xl">Incident center</h1><p className="mt-2 text-sm text-muted-foreground">Kegagalan operasional tujuh hari terakhir tanpa membuka payload atau data bisnis tenant.</p></div>
        ...
        <div className="divide-y divide-border">
          {incidents.map((incident) => (
            <article key={`${incident.kind}-${incident.id}`} className="grid gap-4 p-5 hover:bg-muted/35 md:grid-cols-[3.5rem_1fr_12rem_8rem] md:items-center md:px-6">
              ...
              <div><p className="text-xs font-bold">{incident.tenantName}</p><p className="mt-1 font-mono text-[10px] text-muted-foreground">{incident.source}</p></div>
              <div className="flex items-center justify-between gap-3 md:block md:text-right"><time className="font-mono text-[10px] text-muted-foreground">{dateTime.format(incident.happenedAt)}</time>{incident.tenantId && <Link href={`/superadmin/tenants/${incident.tenantId}`} aria-label={`Buka ${incident.tenantName}`} className="ml-auto mt-2 flex size-8 items-center justify-center text-muted-foreground hover:bg-muted hover:text-primary"><ArrowUpRight size={14} /></Link>}</div>
            </article>
          ))}
