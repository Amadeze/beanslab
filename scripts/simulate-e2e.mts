...
async function main() {
  ...
  let tenant = await prisma.tenant.findUnique({where: {code: 'KIMAISE'}});
  if (!tenant) {
    tenant = await prisma.tenant.create({data: {name: 'kimaise', code: 'KIMAISE', ownerCapital: 80_000_000, initialCash: 20_000_000}});
  } else {
  ...
  const systemUser = await prisma.user.upsert({
    where:  { email: "evm.dama26@gmail.com" },
    update: { password: systemPassword },
    create: { tenantId: tenant.id, 
      name:     "System",
      email:    "evm.dama26@gmail.com",
